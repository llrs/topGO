...
The default algorithm used by the {\tt topGO} package is a mixture between the {\sf elim} and the
{\sf weight} algorithms and it will be referred as {\sf topgo} (lower-case letters).
...
of this manuscript. The following sections will provide details on the functions used in the sample
...
{\it This object will be the input of the testing procedures ... }.
...
genes and on the chosen ontology.

In our example the running time is quite fast given that we have few genes and the size of the ontology
is relatively small (compared to the BP ontology)
...
define which are the interesting genes, in the {\tt topGOdata} object is that one can apply different group 
test. ....
...
(like Kolmogorov-Smirnov test).

There are two possibilities(interfaces) for applying a test statistic to an object of class
{\tt topGOdata}. The base interface offers a lot of flexibility to the experienced R user allowing him
to implement new test statistics or new algorithms. This interface provides the hart of the testing
procedure in {\tt topGO}. The second interface is more user friendly but also more restrictive in the
choice of the tests and algorithms used. We will further explain how these two interfaces work.


\begin{figure}[!t]
\centering 
\includegraphics[width=.8\linewidth]{topGO_classes_v3}
\caption{The class structure.... }
...
The main function is {\tt getSigGroups()} which takes two arguments. The first argument is an instance of class 
{\tt topGOdata} and the second argument is an instance of class {\tt groupStats} or ....

To distinguish between all the algorithms and to secure that all test
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
Next we show how an instance of the {\tt groupStats} class can be used to perform the test statistic for a
gene set. We will to compute the enrichment of {\it cellular lipid metabolic process}(GO:0044255) term using 
Fisher's exact test. 

First we need to define which is the gene universe, to obtain the genes annotated to GO:0044255 and define the
...
Please note that the {\tt groupStats} class or any ... does not depend on GO, and an object of this class 
can be constructed using any gene universe/gene set. 
...
To use the Kolmogorov-Smirnov (KS) test we need to define a test based on a class which contains the
gene score information 
...
{\tt runTest} function and it can be used only with a predefined set of test statistics and 
...
{\it Since the test statistic can return either a $p$-value or a statistic of the data, we will refer both as scores!}
